...
/* ── Auto-binding de data-action (substitui onclick inline) ── */
document.addEventListener('DOMContentLoaded', function() {
  document.querySelectorAll('[data-action]').forEach(function(el) {
    var action = el.getAttribute('data-action');
    if (!action) return;

    // Remover data-action para evitar re-binding
    el.removeAttribute('data-action');

    el.addEventListener('click', function(e) {
      if (action.indexOf('return false') !== -1) {
        e.preventDefault();
        return;
      }

      // Executar a ação no escopo global
      try {
        var fn = new Function('event', 'with(window) { ' + action + '; }');
        fn.call(window, e);
      } catch(err) {
